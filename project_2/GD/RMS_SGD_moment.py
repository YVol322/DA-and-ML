...
for epoch in range(n_epochs):
    for i in range(m):
        ...
        X_k = X[k*M:(k+1)*M,:]
        y_k = y[k*M:(k+1)*M,:]
        gradients = (2.0/M)* X_k.T @ ((X_k @ betas[j])-y_k)
        G_ada = G_ada + (gradients @ gradients.T)
        G_rms = rho * G_rms + (1-rho) * G_ada
        G_diag = G_rms.diagonal()
        sqrt_G = np.sqrt(G_diag)
        gamma_k = gamma_0/(delta + sqrt_G).reshape(-1,1)
        if j == 0:
            betas.append(betas[j] - np.multiply(gamma_k, gradients))
            j = j+1
        else:
            betas.append(betas[j] - gamma_k * gradients + alpha*(betas[j]-betas[j-1]))
            j = j+1
