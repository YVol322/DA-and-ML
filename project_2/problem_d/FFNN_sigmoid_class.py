...
b_1 = (np.zeros((n_nodes, n_features)) + 0.01)


#initialising the 2nd, ..., L-1 hidden layer weights and biases

W_l = np.random.randn(n_layers, n_nodes, n_nodes)
b_l = (np.zeros((n_layers ,n_nodes, n_features)) + 0.01)

#initialising the output layer weights and biases

W_L = np.random.randn(n_inputs, n_features)
b_L = (np.zeros((n_inputs, n_features)) + 0.01)



#initilising some data

train_pred = np.zeros((n_inputs,1))   #initilising the prediction of NN
...
def Back_propagation(y):
    ...
    for j in range(n_layers+2):
        W_list[-1-j] = W_list[-1-j] - gamma * delta_list[-1-j] @ a_list[-1-j].T #  - lmd *
