...
for i in range(1, maxdegree+1, 1):
    ...
    beta_scaled = np.linalg.pinv(X_scaled_train.T @ (X_scaled_train) + I) @ (X_scaled_train.T) @ (z_scaled_train)
    ...
    print('Train, Test R2 score')
    print(R2_scaled_train[i-1], R2_scaled_test[i-1])
...
plt.figure(2)
plt.plot(polydegree, R2_scaled_test, label='Test R2')
plt.plot(polydegree, R2_scaled_train, label='Train R2')
plt.xlabel('Polynomial Fit Degree', fontsize = 15)
